...
class YoloLoss(nn.Module):
    ...
    def find_best_iou_boxes(self, pred_box_list: List[Tensor], box_target: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        for b in range(len(pred_box_list)):
            iou = compute_iou(self.xywh2xyxy(
                pred_box_list[b][:, :4]), box_target)  # (N, N)
    ...
    def get_class_prediction_loss(self, classes_pred: Tensor, classes_target: Tensor, has_object_map: Tensor) -> Tensor:
        ...
        class_loss : scalar
        """
        classes_sum = has_object_map * \
            (classes_pred - classes_target).pow(2).sum(dim=-1)  # (batch_size, S, S)
    ...
    def get_no_object_loss(self, pred_boxes_list: List[Tensor], has_object_map: Tensor):
        ...
        for b in range(len(pred_boxes_list)):
            ...
            no_obj_loss += F.mse_loss(conf,
                                      torch.zeros_like(conf), reduction='sum')
    ...
    def forward(self, pred_tensor: Tensor, target_boxes: Tensor, target_cls: Tensor, has_object_map: Tensor):
        ...
        best_ious, best_boxes = self.find_best_iou_boxes(
            pred_boxes_list, has_obj_target_boxes)

        # compute regression loss between the found best bbox and GT bbox for all the cell containing objects
        reg_loss = self.get_regression_loss(
            best_boxes[:, :4], has_obj_target_boxes)

        # compute contain_object_loss
        contain_obj_loss = self.get_contain_conf_loss(
            best_boxes[:, 4].unsqueeze(-1), best_ious)

        # compute final loss
        total_loss += (cls_loss + no_obj_loss +
                       contain_obj_loss + reg_loss) / N

        # construct return loss_dict
        print(
            total_loss.shape,
            reg_loss.shape,
            contain_obj_loss.shape,
            no_obj_loss.shape,
            cls_loss.shape
        )
        loss_dict = dict(
