...
class Agent():
    def __init__(self, action_size):
        ...
        self.optimizer = optim.AdamW(params=self.policy_net.parameters(), lr=learning_rate)
    ...
    def train_policy_net(self, frame):
        ...
        ### CODE ####
        next_qv = self.policy_net(next_states) # (32, 3)
        _, which = next_qv.max(1) # (32,) which action has the largest value
            
        # Find maximum Q-value of action at next state from policy net
        ### CODE ####
        dq = self.target_net(next_states).gather(1, which.unsqueeze(1)).squeeze(1) # (32, )
        eq = rewards + dq * self.discount_factor * mask.to(device)
